...
description: 软件工厂编排流水线——将功能需求转化为完成的、经过测试和验证的实现。通过 7 个专职 Agent 分工协作。支持三种模式：Full（新功能）、Debug（Bug 修复）、Incremental（小改动）。PROACTIVELY invoke when the user wants to build a feature, fix a bug, or make code changes through a structured multi-agent pipeline.
license: MIT
tags:
  - agent
  - orchestration
